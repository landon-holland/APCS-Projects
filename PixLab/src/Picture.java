...
public class Picture extends SimplePicture 
{
  ...
  public void copy(Picture fromPic,
                   int destRow, int destCol, int startRow, int startCol, int endRow, int endCol)
  {
    Pixel fromPixel = null;
    Pixel toPixel = null;
    Pixel[][] toPixels = this.getPixels2D();
    Pixel[][] fromPixels = fromPic.getPixels2D();
    for (int fromRow = startRow, toRow = destRow;
         fromRow < endRow &&
                 toRow < destRow + (endRow - startRow);
         fromRow++, toRow++)
    {
      for (int fromCol = startCol, toCol = destCol;
           fromCol < endCol &&
                   toCol < destCol + (endCol - startCol);
           fromCol++, toCol++)
      {
        fromPixel = fromPixels[fromRow][fromCol];
        toPixel = toPixels[toRow][toCol];
